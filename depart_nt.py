...
class Case():
    def __init__(
            ...
            sx_delay=0,
            xx_delay=0,
            sx_direction='forward',
            xx_direction='forward'
    ):
        self.sx_info = '上行'
        self.xx_info = '下行'
        self.name = _name
        self.sx = sx_trains
        self.xx = xx_trains
        self.sx_delay = sx_delay
        self.xx_delay = xx_delay
        self.sx_direction = sx_direction
        self.xx_direction = xx_direction
...
class Car():
    def __init__(self, _kind, _pics, _info, _length=15, socket_priority=True):
        self.kind = _kind
        self.no = None
        self.length = _length
        self.speed = 60
        self.carindex = 0
        self.Priority = socket_priority
        self.L_src = _pics[0]
        self.R_src = _pics[1]
        self.ZL_src = _pics[2]
        self.ZR_src = _pics[3]
    ...
    def generateCode(self):
        if self.kind[0] == 'J' and self.carindex == 0:
            self.no = _no = '0001'
            _code = '%s%s00AH00000000' % (
                ('%4s' % (self.kind[::-1],))[::-1],
                ('%4s' % (_no[::-1],))[::-1]
            ...
            self.no = _no = str(1000000 + self.carindex)
            _code = '%s%s0000000' % (
                ('%6s' % (self.kind[::-1],))[::-1],
                ('%7s' % (_no[::-1],))[::-1]
            )
        else:
            self.no = _no = str(2000000 + self.carindex)
            _code = 'T%s%s000000' % (
                ('%6s' % (self.kind[::-1],))[::-1],
                ('%7s' % (_no[::-1],))[::-1]
            )
...
class Loader(QtWidgets.QDialog):
    def __init__(self, _sender, parent=None):
        ...
        if self.eventSender.lastRightSender.objectName() == 'lst_case':
            self._type = CONSTVALUES.DATA_TYPE_CASE
        elif self.eventSender.lastRightSender.objectName() == 'tr_new_case_sx':
            self._type = CONSTVALUES.DATA_TYPE_TRAIN
        elif self.eventSender.lastRightSender.objectName() == 'tr_new_case_xx':
            self._type = CONSTVALUES.DATA_TYPE_TRAIN
        elif self.eventSender.lastRightSender.objectName() == 'tr_new_train':
            self._type = CONSTVALUES.DATA_TYPE_CAR
        ...
        try:
            if self.eventSender.lastRightSender.objectName() == 'lst_case':
                for i in range(self.listView_2.count()):
                    ...
                    _case = self.eventSender.logic.case_mgr[_item]
                ...
                for i in range(self.listView_2.count()):
                    _item = self.listView_2.takeItem(0).text()
                    _train = self.eventSender.logic.train_mgr[_item]
                    _root = QtWidgets.QTreeWidgetItem(self.eventSender.lastRightSender)
                    _root.setText(0, str(_train.name))
                    _root.setText(1, str(len(_train.car)))
    ...
    def data_init(self):
        if self._type == CONSTVALUES.DATA_TYPE_CASE:
            try:
                ...
                pass
        elif self._type == CONSTVALUES.DATA_TYPE_TRAIN:
            try:
                for _train in list(self.eventSender.logic.train_mgr.keys()):
                    _item = QtWidgets.QListWidgetItem(_train)
                    self.listView.addItem(_item)
            except Exception as e:
                pass
        elif self._type == CONSTVALUES.DATA_TYPE_CAR:
            try:
...
class logic():
    ...
    def _get_pic_info(self, file_name):
        import PIL.Image
